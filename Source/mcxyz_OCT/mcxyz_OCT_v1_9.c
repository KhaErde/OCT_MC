...
long	i,j,NN,Nyx,k,ls_cnt;         /* dummy indices */
...
float 	zsurf, Rd;
//float x_start, y_start, z_start; /*launch position from matlab*/
...
double h_step_ini; // distance of each photon step size in pixel
...
int main(int argc, const char * argv[]) 
{
    ...
    //fgets(buf, 32,fid);
	//sscanf(buf, "%f", &x_start);  // x_start
    //fgets(buf, 32,fid);
	//sscanf(buf, "%f", &y_start);  // y_start
    //fgets(buf, 32,fid);
	//sscanf(buf, "%f", &z_start);  // z_start
    ...
    printf("lambda = %0.8f [cm]\n",lambda);
    printf("f = %0.6f [cm]\n",f);
    printf("D = %0.6f [cm]\n",D);
    printf("z_f_img = %0.6f [cm]\n",z_f_img);    
    printf("h_step_ini = %0.6f [cm]\n",h_step_ini);   
    ...
    //Track_x = malloc(sizeof(double)); // track x position
    //Track_y = malloc(sizeof(double)); // track y position
    //Track_z = malloc(sizeof(double)); // track z position
	// read binary file
    strcpy(filename,myname);
       ...
       // printf("i_photon = %ld\n",i_photon); // KE: check i_photon
		...
		{ 
            ...
            { // gaussian source 
                ...
                w_surf = w_0*sqrt(1+((z_f-zs)/z_R)*((z_f-zs)/z_R));
                //sigma_surf = w_surf/2;
                sigma_surf =  2 * w_surf;
                ...
                //printf("y = %0.6f [cm]\n",y);
                //printf("z = %0.6f [cm]\n",z);
                //printf("z_f_left = %0.6f [cm]\n",z_f_left);
                //printf("h_step = %0.6lf [cm]\n",h_step);
		 ...
		 *******/
		do { //while (photon_status == ALIVE || cont_exist == 1 );  /* end STEP_CHECK_HOP_SPIN */
			/**** HOP
			...
			do{  // while sleft>0 
                /*
                Track_x = realloc(Track_x,(k+1)*sizeof(double));// comment the whole tracks
                Track_x[k] = x;
                Track_y = realloc(Track_y,(k+1)*sizeof(double));
                Track_y[k] = y;
                Track_z = realloc(Track_z,(k+1)*sizeof(double));
                Track_z[k] = z;
                */
                //if (sleft < h_step)
                //{
                ...
                {
                    //printf("first_scattering_occured = %d\n",first_scattering_occured);
                    if (h_step > sleft)
                    {
                        h_step = sleft;
                        //printf("h_step = %lf\n",h_step);
                    ...
                    R = -(z_f_left*nr) * (1+(z_R/(z_f_left*nr))*(z_R/(z_f_left*nr)));
                    //printf("z_f_left = %f\n",z_f_left);
                    //printf("R = %f\n",R);
				...
				else /* photon has crossed voxel boundary */
				{
                    ...
                    ls_cnt += 1;
                    //printf("photon crossed boundary\n");
    ...
    // Save the binary file
    strcpy(filename,myname);
    strcat(filename,"_Track_x.bin");
    printf("saving %s\n",filename);
    fid = fopen(filename, "wb");   /* 3D voxel output */
    /*fwrite(Track_x, sizeof(double), k, fid);
    fclose(fid);

    // Save the binary file
    strcpy(filename,myname);
    strcat(filename,"_Track_y.bin");
    printf("saving %s\n",filename);
    fid = fopen(filename, "wb");   /* 3D voxel output */
    /*fwrite(Track_y, sizeof(double), k, fid);
    fclose(fid);

    // Save the binary file
    strcpy(filename,myname);
    strcat(filename,"_Track_z.bin");
    printf("saving %s\n",filename);
    fid = fopen(filename, "wb");   /* 3D voxel output */
    /*fwrite(Track_z, sizeof(double), k, fid);
    fclose(fid);
    */
